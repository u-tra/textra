...
use crate::{load_config, show, watch_config, AppMessage, AppState, Replacement, TextraConfig, MAX_TEXT_LENGTH};
...
pub fn main_loop(app_state: Arc<AppState>, receiver: &std::sync::mpsc::Receiver<AppMessage>) -> Result<()> {
    while let Ok(msg) = receiver.recv() {
        match msg {
            AppMessage::KeyEvent(vk_code, w_param, l_param) => {
